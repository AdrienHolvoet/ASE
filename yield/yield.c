...
enum etat_t { READY, ACTIVABLE, TERMINATED };
...
void init_list() {
        init_ctx(&ctx_ping, 0x4000, ping, NULL);
        init_ctx(&ctx_pong, 0x4000, pong, NULL);
        init_ctx(&ctx_pang, 0x4000, pang, NULL);
        ctx_ping.next = &ctx_pong;
        ctx_pong.next = &ctx_pang;
        ctx_pang.next = &ctx_ping;
